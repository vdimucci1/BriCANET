...
def fuse_color_spaces(image):
    ...
    # Separate channels
    h, s, v = cv2.split(np.float32(hsv_image))
    l, a, b = cv2.split(np.float32(lab_image))

    # Fuse channels
    fused_image_f = cv2.merge((h + l, s + a, v + b))
    fused_image = np.uint8(np.round((fused_image_f / 512.0) * 255.0))
    return fused_image
